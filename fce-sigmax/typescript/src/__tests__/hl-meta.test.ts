/**
 * Vendored unchanged from packages/agent/test/hyperliquid-meta.test.ts (only the import path differs).
 * The market math was copied into the enclave rather than reimplemented, so its proof travels with it:
 * if the vendored copy ever drifts from the original, these assertions are what catches it.
 */
